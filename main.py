...
def test1(evolutionmodel,initsize,methods,duration=None):
    ...
    for r in range(4,1,-1):
        ...
        # for i in range(2):
        #     poolist1.append(p1.apply_async(dynamicMethods, args=(copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), initsize, evolutionmodel, methods, r,coverS,)))
        # p1.close()
        # p1.join()

        tmp,cost=dynamicMethods(copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), initsize, evolutionmodel, methods, r, coverS,duration=duration)
...
def dynamicMethods(nodeAttribute,nodeNeighbors,initsize,evolutionmodel,methods,r,coverS,edges=None,avedeg=None,opinion=None,duration=3):
    count=0
    Cost=[0,0]   #jump, walk
    a = 0
    interval = 1
    while len(coverS.CS)<len(nodeAttribute.keys()):
        ...
        if avedeg:
            ...
            nodesInvolved=cm.networkEvolution(nodeAttribute,nodeNeighbors,initsize,a,interval,coverS,evolutionmodel,edges=edges)
...
def statiCases(evolutionmodel,endsize,r=100):
    ...
    # p1 = multiprocessing.Pool(2)
        coverS = cm.coverSet(r)
        tmp,cost,opinion=cm.AlternateRandom2(copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), copy.deepcopy(coverS),evolutionmodel=evolutionmodel,initsize=endsize)
...
def Random(evolutionmodel,startsize):
    ...
    for r in range(2,1,-1):
        ...
        # p1 = multiprocessing.Pool(2)
        coverS = cm.coverSet(r)
        tmp,opinion=cm.Random(copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), coverS, evolutionmodel, startsize, None,
